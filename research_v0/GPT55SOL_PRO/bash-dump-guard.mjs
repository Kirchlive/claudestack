 ...
 *
 * Capability-gated Claude Code Bash output reducer.
 *
 * Key properties:
...
const SECRET_RULES = [
  ...
  { name: 'openai-key', re: /\bsk-(?:proj-)?[A-Za-z0-9_-]{20,}\b/g, replacement: '[REDACTED API KEY]' },
  ...
  { name: 'bearer', re: /\b(Bearer\s+)[A-Za-z0-9._~+\/-]{16,}/gi, replacement: '$1[REDACTED]' },
  {
    name: 'named-secret',
    re: /\b(api[_-]?key|access[_-]?token|auth[_-]?token|client[_-]?secret|secret|password|passwd)\b\s*[:=]\s*(["']?)([^\s"'`,;]{8,})\2/gi,
...
function selfTest() {
  ...
  try {
    ...
    console.log('bash-dump-guard v3 self-test: OK');
  } finally {
