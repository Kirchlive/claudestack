...
- Kein `PreToolUse:Bash`-Mutator im Default. Falls später aktiviert: höchstens ein
  Command-Rewriter.
- `hooks/claudestack.mjs` ist genau ein, alleiniger aktiver Bash-Output-Owner
  für `PostToolUse:Bash`.
- Read-Guard und Bash-Output-Guard laufen über denselben lokalen Dispatcher.
...
| `SHA256SUMS.txt` | Prüfsummen aller ausgelieferten Paketdateien außer Manifest selbst |
