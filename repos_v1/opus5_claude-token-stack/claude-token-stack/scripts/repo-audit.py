...
D (Evidenzlage) und E (Hebel) sind Kodierungen aus den Validierungsdatensaetzen und
stehen in evidence.json; sie werden hier nur eingelesen, nicht gemessen.

    python3 repo-audit.py --repos repos.txt --out audit.json
...
def score(d, evidence=None):
    """A (30) + B (20) + C (15) gemessen, D (20) + E (15) aus evidence.json."""
    out = {}
    ...
    ev = (evidence or {}).get(d["repo"], {})
    n = ev.get("datasets_recommending", 0)
    tier = ev.get("evidence_tier", 1)
    out["D_evidenz"] = {0: 2, 1: 8, 2: 13, 3: 17, 4: 20}.get(n, 2)
    out["E_hebel"] = {3: 15, 2: 11, 1: 7}.get(tier, 7)
    out["score"] = out["A_lieferfaehigkeit"] + out["B_wartungslast"] + out["C_recht"] + out["D_evidenz"] + out["E_hebel"]
...
def main():
    ...
    ap.add_argument("--evidence", help="evidence.json mit D/E je Repo")
    ...
    evidence = json.load(open(a.evidence, encoding="utf-8")) if a.evidence else {}

    rows = []
    with cf.ThreadPoolExecutor(max_workers=a.workers) as ex:
        for d in ex.map(measure, repos):
            d.update(score(d, evidence))
            rows.append(d)
    rows.sort(key=lambda r: -r["score"])
    json.dump({"generated": TODAY.isoformat(), "count": len(rows),
               "method": "HTTP-Scrape github.com; A/B/C gemessen, D/E kodiert",
    ...
    for r in rows:
        print(f"{r['repo']:<40}{r['A_lieferfaehigkeit']:>4}{r['B_wartungslast']:>4}{r['C_recht']:>4}"
              f"{r['D_evidenz']:>4}{r['E_hebel']:>4}{r['score']:>5}  {','.join(r['flags'])}")
    print(f"\ngeschrieben: {a.out}")
