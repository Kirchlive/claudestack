...
import { resolveInConfigDir } from './lib/nudge-budget.mjs';
...
const DEFAULTS = Object.freeze({
  ...
  // Aggregierter Deckel ueber die lokal gemessenen Prefix-Flaechen (OPUS-Port).
  maxPrefixEstimatedTokens: 6_000,
  ...
  includeGlobal: true,
  includeProject: true,
  // Relativ zum Claude-Konfigverzeichnis aufgeloest (resolveInConfigDir), damit alle
  // Token-Stack-Dateien unter <configDir>/token-stack/ gebuendelt bleiben und mit
  // CLAUDE_CONFIG_DIR mitwandern. Absolute Pfade und ~-Praefixe bleiben unveraendert.
  reportFile: 'token-stack/prefix-latest.json',
  metricsFile: 'token-stack/prefix-snapshots.jsonl',
  metricsMaxBytes: 5_242_880,
  configFile: 'token-stack/prefix-budget.config.json',
...
function loadConfig(cwd) {
  ...
  const configuredPath = resolveInConfigDir(process.env.PREFIX_BUDGET_CONFIG || DEFAULTS.configFile, { configDir, cwd });
  ...
  config.reportFile = resolveInConfigDir(process.env.PREFIX_BUDGET_REPORT || config.reportFile, { configDir, cwd });
  config.metricsFile = resolveInConfigDir(process.env.PREFIX_BUDGET_METRICS || config.metricsFile, { configDir, cwd });
  return config;
}
...
// OPUS-Port, Defekt D1 (2026-08-13): die Vorfassung gruppierte Kollisionen auf dem
// ROHEN Matcher-String. Damit galten 'Bash' und 'Bash|Read|Grep' als zwei getrennte
// Flaechen, obwohl beide auf Bash feuern — die Kollision, die der Waechter finden
// soll, wurde unterzaehlt (OPUS5 mass auf der realen Konfiguration 2 statt 3
// Handler auf PreToolUse:Bash). Der Matcher wird jetzt in die Menge der Tools
// aufgeloest, auf die er tatsaechlich feuert, und je Tool gezaehlt.
//
// Aufloesungsregeln nach Claude-Code-Doku:
//   '*' | '' | fehlend           -> alle Tools (Wildcard)
...
export function matcherToTools(matcher) {
  ...
  const hits = KNOWN_TOOLS.filter((tool) => re.test(tool));
...
function buildFindings(snapshot, config) {
  ...
  // OPUS-Port inkl. D1-Fix: Hook-Flaechen-Kollision (Gesetz I — genau ein mutierender
  // Eigentuemer pro Fläche; passende Hooks laufen parallel und sind nicht komponierbar).
  // Gruppiert wird ueber die aufgeloeste Tool-Menge, nicht ueber den Matcher-String.
  const surfaces = new Map();
  for (const hook of snapshot.hooks.registered) {
    if (!/^(PreToolUse|PostToolUse)$/.test(hook.event)) continue;
    for (const tool of matcherToTools(hook.matcher)) {
      const key = `${hook.event}:${tool}`;
      if (!surfaces.has(key)) surfaces.set(key, []);
      surfaces.get(key).push(hook.command);
    }
  }
  // Wildcard-Handler feuern auf jede konkrete Flaeche und zaehlen dort mit.
  const wildcards = new Map();
  for (const [key, commands] of surfaces) {
    const [event, tool] = key.split(':');
    if (tool === '*') wildcards.set(event, commands);
  }
  for (const [key, commands] of surfaces) {
    const [event, tool] = key.split(':');
    if (tool === '*') continue;
    const all = [...new Set([...commands, ...(wildcards.get(event) || [])])];
    if (all.length > 1) add('high', 'hook-surface-collision', `${all.length} hooks registered on ${key} run concurrently; mutation results are not composable (Gesetz I: exactly one mutating owner per surface).`, 'settings declaration; matcher resolved to tool set (D1)');
  }
  for (const [event, commands] of wildcards) {
    const unique = [...new Set(commands)];
    if (unique.length > 1) add('high', 'hook-surface-collision', `${unique.length} wildcard hooks registered on ${event}:* run concurrently (Gesetz I).`, 'settings declaration; wildcard matcher');
  }
  // OPUS-Port: aggregiertes Prefix-Budget. Bewusst NUR ueber lokal gemessene Posten —
  // OPUS' Stueckkostenmodell (n Skills x ~100 Token, n MCP-Server x Schaetzwert) ist
  // von OPUS selbst gestrichen worden und wird hier nicht uebernommen.
  if (snapshot.prefixTotal.estimatedTokens > config.maxPrefixEstimatedTokens) {
    add('high', 'prefix-total-over-budget', `Measured prefix surfaces total ~${snapshot.prefixTotal.estimatedTokens} estimated tokens versus a ${config.maxPrefixEstimatedTokens}-token budget.`, `sum of ${snapshot.prefixTotal.components.join(' + ')}; excludes ${snapshot.prefixTotal.excludes.join(', ')}`);
  }
...
function buildSnapshot(cwd, config) {
  ...
  // OPUS-Port: eine Gesamtzahl statt nur Einzelbefunde. Summiert werden ausschliesslich
  // lokal gemessene Posten; alles Ungemessene steht in `excludes` und wird NICHT
  // geschaetzt (kein Stueckkostenmodell — siehe buildFindings).
  snapshot.prefixTotal = {
    estimatedTokens: snapshot.skills.estimatedMetadataTokens + snapshot.markdown.claudeEstimatedTokens,
    components: ['skill metadata listing', 'candidate CLAUDE.md files'],
    excludes: ['MCP tool schemas', 'path-scoped rule files', 'system prompt', 'built-in tool definitions'],
    budgetTokens: config.maxPrefixEstimatedTokens
  };
...
function selfTest() {
  ...
  try {
    ...
    // Der PreToolUse-Fall ist der D1-Regressionstest: 'Bash' und 'Bash|Read|Grep'
    // sind verschiedene Matcher-Strings, feuern aber beide auf Bash. Eine Gruppierung
    // auf dem rohen String findet die Kollision NICHT.
    fs.writeFileSync(path.join(fakeHome, '.claude', 'settings.json'), JSON.stringify({
      ...
      hooks: {
        ...
        PreToolUse: [
          { matcher: 'Bash', hooks: [{ command: 'gate.mjs' }] },
          { matcher: 'Bash|Read|Grep', hooks: [{ command: 'wide.mjs' }] }
        ]
      }
    }));
    ...
    // D1-Regression: die Alternation 'Bash|Read|Grep' muss auf PreToolUse:Bash mitzaehlen.
    // Ohne den Fix bleibt dieser Befund aus, weil die Matcher-Strings verschieden sind.
    if (!snapshot.findings.some((item) => item.code === 'hook-surface-collision' && /PreToolUse:Bash\b/.test(item.message))) {
      throw new Error('D1 regression: alternation matcher not resolved onto PreToolUse:Bash');
    }
    // Die Alternation darf ausserdem auf Read und Grep zaehlen, dort aber allein stehen.
    if (snapshot.findings.some((item) => item.code === 'hook-surface-collision' && /PreToolUse:(Read|Grep)\b/.test(item.message))) {
      throw new Error('D1 regression: single handler on Read/Grep reported as collision');
    }
    // Aggregiertes Prefix-Budget (OPUS-Port) muss eine Gesamtzahl fuehren.
    if (!Number.isFinite(snapshot.prefixTotal?.estimatedTokens)) throw new Error('Missing aggregated prefix total (OPUS port)');
    writePrivateJson(config.reportFile, snapshot);
    if (!fs.existsSync(config.reportFile)) throw new Error('Report not written');
    console.log('prefix-budget self-test: OK (inkl. OPUS-Kollisionserkennung und D1-Regression)');
  } finally {
