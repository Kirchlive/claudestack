...
import { claudeConfigDir, resolveInConfigDir, tryConsumeNudge } from './lib/nudge-budget.mjs';

const VERSION = 1;
// Pfadvorgaben relativ zum Claude-Konfigverzeichnis (CLAUDE_CONFIG_DIR,
// Vorgabe ~/.claude) — siehe read-context-guard, AP-4.5.
const DEFAULTS = Object.freeze({
  enabled: true,
  configFile: 'token-stack/session-economy.config.json',
  stateDir: 'token-stack/session-economy',
  checkpointDir: 'token-stack/checkpoints',
  ...
  retentionDays: 14,
  pruneChance: 0.02,
  // Ueberschreibt das gemeinsame Nudge-Budget (lib/nudge-budget.mjs), z. B.
  // { maxPerSession: 2 }. Leer = Vorgabe des Moduls.
  nudge: {}
});
...
function loadConfig(cwd = process.cwd()) {
  ...
  const globalFile = resolveInConfigDir(process.env.SESSION_ECONOMY_CONFIG || DEFAULTS.configFile, { configDir, cwd });
  ...
  config.stateDir = resolveInConfigDir(process.env.SESSION_ECONOMY_STATE_DIR || config.stateDir, { configDir, cwd });
  config.checkpointDir = resolveInConfigDir(process.env.SESSION_ECONOMY_CHECKPOINT_DIR || config.checkpointDir, { configDir, cwd });
...
async function main() {
  ...
  if (event === 'Stop') {
    const band = crossedBand(snapshot, state, config);
    if (band !== null) {
      // Gemeinsames Nudge-Budget (AP-4.5): frueher zaehlte dieser Hook nur seine
      // eigenen Baender, bash-size-feedback nur seine eigenen Hinweise — in Summe
      // sah eine Sitzung das Mehrfache des gewollten Deckels. Jetzt buchen beide
      // gegen dieselbe Datei. Das Band wird erst als erledigt vermerkt, wenn der
      // Hinweis wirklich ausgegeben wurde; sonst steht er beim naechsten Stop
      // erneut an, statt still zu verfallen.
      const nudge = tryConsumeNudge(sessionIdFrom(input), 'session-economy', config.nudge);
      if (nudge.granted) {
