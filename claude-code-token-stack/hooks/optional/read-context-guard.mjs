...
import { claudeConfigDir, resolveInConfigDir } from './lib/nudge-budget.mjs';
...
const VERSION = 1;
// Pfadvorgaben sind relativ zum Claude-Konfigverzeichnis (CLAUDE_CONFIG_DIR,
// Vorgabe ~/.claude). Frueher standen hier feste ~/.claude-Pfade — damit landete
// der State neben einer anderen Installation, sobald CLAUDE_CONFIG_DIR gesetzt
// war. prefix-budget loeste das bereits so; AP-4.5 zieht die uebrigen Hooks nach.
const DEFAULTS = Object.freeze({
  enabled: true,
  configFile: 'token-stack/read-context-guard.config.json',
  stateDir: 'token-stack/read-state',
  retentionHours: 48,
  pruneChance: 0.02,
...
function loadConfig(cwd = process.cwd()) {
  ...
  const globalFile = resolveInConfigDir(process.env.READ_CONTEXT_GUARD_CONFIG || DEFAULTS.configFile, { configDir, cwd });
  ...
  config.stateDir = resolveInConfigDir(process.env.READ_CONTEXT_GUARD_STATE_DIR || config.stateDir, { configDir, cwd });
  return config;
}
