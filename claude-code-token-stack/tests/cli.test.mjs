...
import { DEFAULT_CONFIG } from '../src/stack.mjs';

const packageRoot = path.dirname(path.dirname(fileURLToPath(import.meta.url)));
const cli = path.join(packageRoot, 'bin', 'claudestack.mjs');
const hook = path.join(packageRoot, 'hooks', 'claudestack.mjs');

// The shipped default is mirrored four ways: schema, default.json, DEFAULT_CONFIG, and this test.
// The fourth copy used to be a hand-maintained literal here, so a new section (canary) drifted in
// unnoticed. Comparing against the import removes the copy; the section loop below is derived from
// DEFAULT_CONFIG rather than a fixed list so a future section cannot slip past unchecked either.
const defaultConfig = DEFAULT_CONFIG;
const configSections = Object.keys(defaultConfig).filter((key) => (
  defaultConfig[key] !== null && typeof defaultConfig[key] === 'object'
));
...
// ADR-005: enforce without a passed, unexpired probe downgrades to shadow and says so on stderr.
// Every enforce fixture therefore has to carry a valid record, or it tests the downgrade instead.
function writeCapability(configDir) {
  const file = path.join(configDir, 'token-stack', 'capabilities.json');
  fs.mkdirSync(path.dirname(file), { recursive: true });
  const now = Date.now();
  fs.writeFileSync(file, `${JSON.stringify({
    schemaVersion: 1,
    testedAt: new Date(now - 86_400_000).toISOString(),
    expiresAt: new Date(now + 29 * 86_400_000).toISOString(),
...
test('package metadata and JSON configuration artifacts are dependency-free', () => {
  ...
  assert.deepEqual(
    ...
    'schema and DEFAULT_CONFIG must describe the same top-level keys',
  );
  assert.ok(configSections.includes('canary'), 'canary must be covered by the section loop');
