...
- bash-dump-guard ersetzt ohne bestandene Canary-Probe nichts (Shadow-Modus —
  Absicht, kein Defekt).
- toonify-mcp nur mit **Pin ≥ 0.8.2** (additionalContext-Defekt erst dort gefixt; Burst-Warnung: 4 Releases an einem Tag — Changelog-Pflicht).

**Namensklärung (META-3RUNDE §3):** Es existieren drei verschiedene Artefakte namens `bash-dump-guard.mjs`: (1) das Deny-Gate aus dem Ist-Setup (`hooks/` im Repo, `PreToolUse` — dort korrekt registriert), (2) der GPT-Outputreducer v3 (`PostToolUse`), (3) die Paket-Variante v3.1 (Outputreducer + Canary). Dieses Paket trennt sie sauber: **`bash-dump-gate.mjs`** (Deny-Gate, PreToolUse) vs. **`bash-dump-guard.mjs` v3.1** (Outputreducer, PostToolUse). Die v3.1 liegt in diesem Paket, ist aber noch nicht ins Git-Repo gepusht (Nutzeraktion).
