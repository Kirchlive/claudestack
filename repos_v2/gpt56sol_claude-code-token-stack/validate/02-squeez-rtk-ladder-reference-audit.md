...
- `bash-dump-guard` ist ein Deny-Guard, kein Outputreplacer; der Name suggeriert
  mehr als der gelieferte Code tut.
...
## 11. Nachgeliefertes K3-Supplement

K3 berichtet aus einer zweiten Linux-/FUSE-Umgebung `37/38` Ladder-Fälle; der
eine Fehler entstand wegen fehlendem RTK-Binary. Das referenzierte Paket und die
Rohtranskripte sind im gelieferten ZIP nicht enthalten. Diese Zahl ist daher
getrennt von den lokal reproduzierten `38/38` des ursprünglichen Snapshots zu
führen.

```tsv
metric_id	reported_value	status	grenze
K3_TOKEN_DENSITY	318_token_pro_1000_Zeichen	reported_measurement	19_Kommandoausgaben_ein_Repo
K3_SQUEEZ_LATENCY	86.3_ms_pro_Call	reported_measurement	Rohlaeufe_fehlen
K3_RTK_LATENCY	5.7_ms_pro_Call	reported_measurement	Rohlaeufe_fehlen
K3_CALL_COVERAGE	806_Calls_40.6pct_Bash	reported_measurement	Bash_ist_einziger_Stash-Pfad
K3_MCP_PREFIX	286_Token_pro_Session	reported_measurement	gleicher_Proxy_keine_Providerkosten
K3_RETRIEVE_BREAK_EVEN	47_pct	reported_model	annahmenabhaengig
K3_LADDER_SECOND_ENV	37_pass_1_fail	reported_test	RTK_fehlt_Paket_nicht_mitgeliefert
```

Die K3-Formulierung „Effekt bewiesen“ für v5 wird nicht übernommen. Sechs Läufe
auf drei Quelltext-Strukturfragen ohne mitgelieferte Rohtranskripte bleiben ein
enger positiver Teilflächenbefund, kein allgemeiner Stack-Nachweis.
