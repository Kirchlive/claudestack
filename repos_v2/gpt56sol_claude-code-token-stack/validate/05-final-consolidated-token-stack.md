...
verification:
  package_required_files: 24
  package_required_files_present: 24
  tests_passed: 32
  tests_failed: 0
...
final_decision:
  ...
  invariant:
    ...
    bash_pre_execution_gate: tracked_root_deny_gate_after_policy_review
...
| `A05R` | `06-incoming-reconciliation.md` | korrekter Meta-Intake, Errata-Overlay, Kreuzachsenentscheidung, Guard-Identitäten | kanonische Reconciliation nach allen Nachlieferungen |
...
| `K3` | 2 | 32 | 32 | 3.91 | 63 | verständlicher Best-of-Merge | Revision berichtet Befehle und Suite-Ergebnisse; ausführbares Revisionspaket/Rohlogs fehlen |
...
|---|---|---|
| `FC01` | OPUS-Katalog enthält 376 saubere Repos | v4.1 liefert 375 Einträge, davon 374 im OPUS-Snapshot als `exists:true` markiert, sowie 368 case-insensitiv normalisierte kanonische Namen; Erreichbarkeit wurde nicht neu geprüft; Definitionen und Dublettengruppen offenlegen, nicht als Vollständigkeitsbeleg nutzen |
| `FC02` | OPUS R1–R5 vollständig geliefert | R2 `bash-output-owner.mjs` fehlt |
| `FC03` | K3 liefert unabhängige externe Validierung | ursprüngliches Bündel ohne reproduzierbare Provenienz; Revision berichtet Befehle, Exit-Codes, 73 PASS/0 FAIL und 37/38. Ausführbares Revisionspaket, v3.1-Guard und Rohlogs fehlen: `reported_only`, nicht unabhängig reproduziert |
...
### 5.1 Historische Forschungsflächen vor dem Intake-Abgleich

Diese Tabelle beschreibt den ursprünglichen, breiteren Arbeits-Snapshot und
nicht die fünf am 2026-08-13 nachgelieferten Dateien/ZIPs. Aktuelle
Bundle-Inventare, Hashes und abweichende Scopes stehen kanonisch in
`validate/06-incoming-reconciliation.md` §2 und §4.3.
...
Diese historischen vier Validierungsflächen umfassen 1.071.249 Bytes und
326.397 o200k-Proxy-Token. Das ist Forschungs-/Dateifläche, nicht ein einzelner
Claude-Prompt und nicht der Umfang der nachgelieferten ZIPs.
...
`score_1_100` ist der sicherheits- und korrektheitsorientierte Runtime-Score aus
der kanonischen GPT-Rubrik, kein Lieferfähigkeits- oder Konsensscore. OPUS-
Lieferfähigkeit, Runtime-Korrektheit und Security werden getrennt geprüft;
`replace`/`reject` kann nicht durch Aktivität, Lizenz, Sterne, PR-Metriken oder
einen A4-Mittelwert aufgehoben werden.
...
Die 15-Repo-Kreuzdarstellung in `06-incoming-reconciliation.md` ist die
Entscheidungsform: `ccusage` ist einziges `use`-Werkzeug ohne Runtime-Re-Entry-
Gate. `squeez`, `omni`, `magic-compact` und `claude-code-cache-fix` stehen bei
OPUS bei `>=75`, aber bei GPT auf `replace` oder `reject`. Folge: Defektbehebung
und isoliertes gepaartes A/B, keine Mittelung. Der berichtete Ponytail-Deckel
97→65 bleibt mangels Revisionsformel `reported_only`.

### 6.1 Erhaltener Risikodissens

Kanonische Paketentscheidung bedeutet nicht Modellkonsens. Zwei Unterschiede
bleiben absichtlich ungelöst:

| Surface | GPT56-Runtimepolitik | OPUS5 | K3SWARM | Paketfolge |
|---|---|---|---|---|
| Toonify | Default-`reject`; Re-Entry erst nach Code-, Issue-, Rohwert- und E2E-Audit, dann exakt `v0.8.2` | „freigabefähiger Kern“, als Pilotkandidat normalisiert, Score 85 | konditionaler Format-Pilot, Score 78; Fixmessung nur berichtet | konservatives Reject mit isoliertem Re-Entry |
| Proxy | `llmtrim` und `tokdiet` reject | `llmtrim` konditional nach Cachemessung; `tokdiet` suspendiert | `llmtrim` Hauptoption, `tokdiet` auditpflichtiger Fallback | kein Default-Proxy |

Dies ist Risikodissens über CA-/Proxy-, Cache-, Security- und
Fehlerattributionsgrenzen, kein Rechenfehler. Release-Bestätigung für Toonify
ist Versionsnachweis, kein Funktions- oder Einsparungsnachweis. Vollständige
maschinenlesbare Modellpositionen stehen in `06-incoming-reconciliation.md`.
...
| `bash_pre_execution_gate` | bestehender Root-Deny-Guard nach Policy-Prüfung | weitere deny-only Gates technisch möglich | explizit inventarisieren; kein Input-Mutator |
...
package:
  ...
  posttooluse_bash_owner: null
  posttooluse_bash_candidate: hooks/claudestack.mjs
  candidate_deployment: not_installed
...
verification_result:
  ...
  release_candidate:
    required_files: 24_of_24
    checksum_entries: 32
    checksum_errors: 0
    node_tests: 32_passed_0_failed
    json_configs_parsed: 4
    permission_allow_pattern: absent
    cli_and_hook_syntax: passed
    documentation_contract_checks: 24_of_24
...
7. Regex-, Wildcard- und Komma-Hook-Matcher wurden vom Doctor zunächst
   übersehen: neue Tests rot; vertragsgerechte Auswertung plus
   `INVALID_HOOK_MATCHER`; generische Pfad-/Legacy-Prüfung auf alle Events
   erweitert; disambiguierte GPT55-Owner-ID in Collisionprüfung aufgenommen;
   Gesamt 32/32 grün.
...
| `L10` | K3-Revisionspaket, v3.1-Guard und Rohlogs fehlen | 73/0, 37/38 und Zusatzmetriken bleiben `reported_only` |
| `L11` | OPUS-Revisionsartefakt mit PR-Deckel/Kreuztabelle fehlt | 97→65 nicht als neuer Ponytail-Score übernehmen |
| `L12` | OPUS `D/20` und K3 `Kon/10` bleiben aktive Korpus-Konvergenzterme | diese Scores nicht als unabhängige Runtime-Evidenz verwenden |
